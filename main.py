...
from data import all_processor
...
MODEL_CLASSES = {
    ...
    # 'bert': (RobertaConfig, bertCSRModel, RobertaTokenizer)
}
class CTB_features(object):
    def __init__(self, doc_id,
                 enc_text, enc_tokens, sentences,
                 enc_input_ids, enc_mask_ids, node_event,
                 t1_pos, t2_pos, mh_self_attention_mask, target, rel_type, event_pairs
                 ):
        self.doc_id = doc_id
...
def compute_kl_loss(p, q, pad_mask=None):
    ...
    return loss
def train_epoch(args, model, train_loader, optimizer, scheduler, epoch,tokenizer):
    train_loss = []
    predicted_all = []
    gold_all = []
    clabel_all = []
    ...
    for step, batch in enumerate(train_loader):
        progress.update(1)
        inputs = {'enc_input_ids': batch[0].to('cuda'),
                  'enc_mask_ids': batch[1].to('cuda'),
                  'node_event': batch[2],
                  't1_pos': batch[3],
                  't2_pos': batch[4],
                  'target': batch[5],
                  'rel_type': batch[6],
                  'event_pairs': batch[7],
                  'all_senlable': batch[8],
                  "all_senlable2": batch[9],
                  'type':'train',
                  'doc_id':batch[10],
                  'tokenizer':tokenizer
                  }
        loss, opt= model(**inputs)
        # loss2, opt= model(**inputs)
        # ce_batch_loss = (loss + loss2) / 2
        # kl_batch_loss = compute_kl_loss(loss, loss2)
...
def test_epoch(args, model, test_loader,epoch,tokenizer):
    test_loss = []
    predicted_all = []
    predicted_all1_2 = []
    predicted_all2_1 = []
    ...
    for batch in test_loader:
        ...
        with torch.no_grad():  # 禁用梯度计算，节省内存和加快推理速度
            loss, opt = model(**inputs)
            loss2, opt = model(**inputs)
            ce_batch_loss = (loss + loss2) / 2
            kl_batch_loss = compute_kl_loss(loss, loss2)
...
def valid_epoch(args, model, valid_loader,epoch,tokenizer):
    valid_loss = []
    predicted_all = []
    gold_all = []
    clabel_all = []
...
def cross_validation(args, model_class, processor,tokenizer):
    ...
    for fold, (train_idx, test_idx) in enumerate(splits.split(train_test_dataset)):
        ...
        for epoch in range(1, args.epoch + 1):
            model.zero_grad()
            train_loss, train_r, train_p, train_f1, train_intra, train_cross = train_epoch(args, model, train_loader, optimizer, scheduler, epoch,tokenizer)
            printlog('Epoch{}: Train Loss {} R {} P {} F {}'.format(epoch, train_loss, train_r, train_p, train_f1))
            model.eval()
            with torch.no_grad():
                if epoch % args.test_epoch == 0:
                    test_preds, test_golds, test_clabels, test_loss, test_r, test_p, test_f1, test_intra, test_cross= test_epoch(args, model, test_loader,epoch,tokenizer)

                    printlog('Epoch{}: Test Loss {} R {} P {} F {} intra{} cross{}'.format(epoch, test_loss, test_r, test_p, test_f1, test_intra, test_cross))

                if epoch % args.valid_epoch == 0:
                    valid_loss, valid_r, valid_p, valid_f1, valid_intra, valid_cross = valid_epoch(args, model, valid_loader, epoch,tokenizer)
                    printlog('Epoch{}: Valid Loss {} R {} P {} F {} intra{} cross{}'.format(epoch, valid_loss, valid_r, valid_p, valid_f1, valid_intra, valid_cross))
                    if args.dataset_type=='CTB':
                        if valid_intra['f1'] >= best_valid_f1:
                            best_valid_f1 = valid_intra['f1']
    ...
    # printlog("\n-------------------------Causality Direction Identification1_2-------------------------\n")
    # printlog("--------  \t Intra: P {} R {} F1 {} \n\t Cross: P {} R {} F1 {}  \n\t 5-Fold Avg: P {} R {} F1 {}---------".format(np.mean(avg_test_intra_p1), np.mean(avg_test_intra_r1), np.mean(avg_test_intra_f11), np.mean(avg_test_cross_p1), np.mean(avg_test_cross_r1),
    #                                                                                                               np.mean(avg_test_cross_f11), np.mean(avg_test_p1), np.mean(avg_test_r1), np.mean(avg_test_f11)))
    # printlog("\n-------------------------Causality Direction Identification2_1-------------------------\n")
    # printlog("--------  \t Intra: P {} R {} F1 {} \n\t Cross: P {} R {} F1 {}  \n\t 5-Fold Avg: P {} R {} F1 {}---------".format(np.mean(avg_test_intra_p2), np.mean(avg_test_intra_r2), np.mean(avg_test_intra_f12), np.mean(avg_test_cross_p2), np.mean(avg_test_cross_r2),
    #                                                                                                               np.mean(avg_test_cross_f12), np.mean(avg_test_p2), np.mean(avg_test_r2), np.mean(avg_test_f12)))
...
def main():
    ...
    args.log = str(args.log) + '/t_lr' + str(args.t_lr) + 'lr' + str(args.lr) + t + str(args.method)+'.txt'
    ...
    if args.dataset_type=='all':
        processor = all_processor(args, tokenizer, printlog)
    else:
        processor = ESC_processor(args, tokenizer, printlog)

    # processor = MAVEN_processor(args, tokenizer, printlog)

    printlog("Training/evaluation parameters :{}".format(args))
    cross_validation(args, eci_model_class, processor,tokenizer)
